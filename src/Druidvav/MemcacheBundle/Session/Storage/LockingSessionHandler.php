<?php
namespace Druidvav\MemcacheBundle\Session\Storage;
...
class LockingSessionHandler implements \SessionHandlerInterface
{
    ...
    /**
     * @var MemcachedClient Memcache driver.
     */
    private $memcache;
     ...
     *
     * @param MemcachedClient $memcache  A MemcachedClient instance
     * @param array         $options   An associative array of Memcache options
     *
     * @throws \InvalidArgumentException When unsupported options are passed
     */
    public function __construct(MemcachedClient $memcache, array $options = array())
    {
    ...
    private function lockSession($sessionId)
    {
        ...
        for ($i=0;$i<$attempts;$i++) {
            $success = $this->memcache->add($this->prefix.$this->lockKey, '1', $this->lockMaxWait+1);
            if ($success) {
                $this->locked = true;
                return true;
            }
    ...
    public function write($sessionId, $data)
    {
        ...
        return $this->memcache->set($this->prefix.$sessionId, $data, $this->ttl);
    }
